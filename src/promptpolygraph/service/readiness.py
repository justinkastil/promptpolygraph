...
def check(store: SqlStore, settings: Settings, llm_probe: Probe | None = None) -> tuple[bool, list[dict[str, Any]]]:
    ...
    try:
        ...
        report.append({"check": "store", "status": "fail", "detail": "store probe failed"})
    ...
    try:
        ...
        report.append({"check": "queue", "status": status, "depth": depth, "maximum": settings.queue_max_depth})
    except Exception:
        report.append({"check": "queue", "status": "fail", "detail": "queue probe failed"})
    ...
    if not settings.readiness_llm_check:
        ...
        try:
            ...
            report.append({"check": "llm", "status": "fail", "detail": "LLM probe failed"})
