...
def test_retry_is_exponential_then_dead_letters_and_manual_retry_validates(tmp_path):
    ...
    second = record_failure(store, seeded["job_id"], "two")
...
def test_drain_timeout_is_bounded_and_state_stays_latched():
...
def test_retention_preserves_running_job_and_its_run(tmp_path):
...
def test_snapshot_can_be_reopened_and_queried(tmp_path):
    store = _store(tmp_path)
    seeded = store.enqueue_job({"mock": True})
