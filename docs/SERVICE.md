...
`POLYGRAPH_STARTUP_LLM_CHECK` is off by default; embeddings may supply an
offline-testable probe when opting into that check.

### Kubernetes health probes

`GET /healthz` is process **liveness** only. It intentionally remains healthy
when a dependency is unavailable. `GET /healthz/ready` is **readiness**: it
checks the database/store, ensures the queued-job count is no greater than
`POLYGRAPH_QUEUE_MAX_DEPTH` (default `1000`), and can include an opt-in LLM
probe with `POLYGRAPH_READINESS_LLM_CHECK=true`. A failed readiness dependency
returns HTTP `503`.
